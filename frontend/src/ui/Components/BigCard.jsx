    ...
    <Link to={`/movie/${id}`} onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
      <div className="">
        <div className="relative aspect-[0.75] rounded-md w-64">
          ...
          {isHovered && (
            <div className="absolute top-0 left-0 right-0 bottom-0 bg-neutral-900 bg-opacity-50 rounded-md flex flex-col justify-between items-end p-4">
              {isAuthenticated ? (
              ...
              <div className="text-neutral-100 text-sm w-full flex flex-col gap-1" onclick="">
